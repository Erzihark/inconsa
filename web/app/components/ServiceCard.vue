...
<template>
  <div
    class="group relative flex h-full flex-col overflow-hidden bg-surface p-7 shadow-sm ring-1 ring-ink/5 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl"
  >
    ...
    />
    <div
      class="mb-5 flex h-14 w-14 items-center justify-center bg-ink text-accent transition-colors duration-300 group-hover:bg-accent group-hover:text-ink"
    >
    ...
    <h3 class="font-display text-2xl tracking-wide text-ink">{{ loc(service.title) }}</h3>
    <p v-if="loc(service.description)" class="mt-2 font-subtitle text-sm leading-relaxed text-ink/70">
